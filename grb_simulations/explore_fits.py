...
class Support:
    ...
    def simulation_run(self, output_events_file, model_file, time=[0, 1800], energy=[None, None], seed=None, log_file='ctobssim.log'):
        sim = ctools.ctobssim()
        sim.clear()
        ...
        sim.run()
        sim.save()
        print("Events file '{}' created (time [{}-{}]).".format(output_events_file, time[0], time[1]), file=sys.stderr)
...
if __name__ == "__main__":
    ...
    for s in slices:
        ...
        if args.tmax is not None:
            ...
            if tstop > args.tmax:
                tstop = args.tmax
        create_events_file(id=s['slice'], model_file=s['model_file'], tmin=tstart, tmax=tstop)
        tstart=tstop
    exit(0)
