...
def data_augmentation(data, bins_number=50):
    ...
    for data_name, d in data.items():
        ...
        for f in fields:
            f_name = f['name']
            data_arr_ref = d['{}_array'.format(f_name)]
            ...
            d['stats'][f_name] = {
                ...
                'stdev': data_stats['stdev'],
                'median': data_stats['median'],
            }

            starting_parameters = [1., data_stats['mean'], data_stats['stdev']] #, 0.] # A, mu, sigma, Q
            fit_coeff_norm, pvalue_err_norm = fitting_data(gauss, initial_params=starting_parameters, x=bins_centres, y=counts_hist_normalized, verbosity=False, name=data_name)
...
def create_hist(ax, data, data_stats, xlabel=None, color="blue", dyn_bins=False, density_flag=True):
    bins_centres = data['bins_centres']
    bins_width   = data['bins_width']
    fit_params_norm = data['fit_coeff_norm']
    fitted_hist_norm = gauss(bins_centres, *fit_params_norm)
    counts_hist_norm = data['counts_norm']

    if density_flag:
        ax.bar(bins_centres, height=counts_hist_norm, width=bins_width, alpha=0.5, edgecolor=color, color=color, label='data')
    ...
    ax.axvline(data_stats["mean"], color="orange", linestyle="--", alpha=0.9)
    # ax.axvline(sts["median"], color="black",  linestyle="-.", alpha=0.5)

    if xlabel is not None:
        ax.set_xlabel('{} (bins: {})'.format(xlabel, data['n_bins']))
...
def plot_data_summary(data):
    ...
    for d in data:
        ...
        create_hist(ax[0][1], d['hist']['N_on'], d['stats']['N_on'], color="magenta", xlabel="N on", dyn_bins=True)
        create_hist(ax[1][1], d['hist']['N_off'], d['stats']['N_off'], color="red", xlabel="N off", dyn_bins=True)
        create_hist(ax[2][1], d['hist']['N_s'], d['stats']['N_s'], color="yellow", xlabel="N signal", dyn_bins=True)

        # Important: first tight_layout(), after adjust for the title
        fig.tight_layout()
        fig.subplots_adjust(top=0.95)
        # plt.show()
        plt.savefig("{0}_{1:04d}.png".format(d["name"], d["tmax"]), format="png")
        plt.close()
        print("saving {0}_{1:04d}.png".format(d["name"], d["tmax"]), file=sys.stderr)
    return None
